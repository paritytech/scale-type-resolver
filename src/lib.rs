...
// TODO: Monday: Have a go at writing a nice macro to make it easy to declare lambdas for ResolveTypeVisitor.
// resolve_visitor!(
//     'resolver;

//     Value = Result<(), Error>;

//     visit_composite = |fields| {

//     },
//     visit_unhandled = |unhandled| {
...
mod builder {
    ...
    pub struct ResolvedTypeVisitorBuilder<
        TypeId,
        Unhandled,
        NotFound,
        Composite,
        ...
        BitSequence,
    > {
        _marker: core::marker::PhantomData<(TypeId,)>,
        visit_unhandled: Option<Unhandled>,
        visit_not_found: Option<NotFound>,
        visit_composite: Option<Composite>,
        visit_variant: Option<Variant>,
        visit_sequence: Option<Sequence>,
        visit_array: Option<Array>,
        visit_tuple: Option<Tuple>,
        visit_primitive: Option<Primitive>,
        visit_compact: Option<Compact>,
        visit_bit_sequence: Option<BitSequence>,
    }
    ...
    > {
        pub fn new() -> Self {
            ResolvedTypeVisitorBuilder {
                ...
                visit_unhandled: None,
                visit_not_found: None,
                visit_composite: None,
                visit_variant: None,
                visit_sequence: None,
                visit_array: None,
                visit_tuple: None,
                ...
                visit_compact: None,
                visit_bit_sequence: None,
            }
        }
    }

    fn test() {
        let f = ResolvedTypeVisitorBuilder::new();
    ...
    impl <
        ...
        BitSequenceFn,
        Output,
    > ResolvedTypeVisitor<'resolver> for ResolvedTypeVisitorBuilder <
        TypeId,
        UnhandledFn,
        NotFoundFn,
        CompositeFn,
    ...
    > where
        ...
        UnhandledFn: FnOnce(UnhandledKind) -> Output,
        NotFoundFn: FnOnce() -> Output,
        CompositeFn: FnOnce(&mut dyn FieldIter<'resolver, TypeId>) -> Output,
        VariantFn: FnOnce(&mut dyn VariantIter<'resolver, ConcreteFieldIter<'resolver, TypeId>>) -> Output,
        SequenceFn: FnOnce(&'resolver TypeId) -> Output,
        ArrayFn: FnOnce(&'resolver TypeId, usize) -> Output,
        TupleFn: FnOnce(&mut dyn ExactSizeIterator<Item = &'resolver TypeId>) -> Output,
        PrimitiveFn: FnOnce(Primitive) -> Output,
        CompactFn: FnOnce(&'resolver TypeId) -> Output,
        BitSequenceFn: FnOnce(BitsStoreFormat, BitsOrderFormat) -> Output,
        Output: Default,
    {
        type TypeId = TypeId;
        type Value = Output;
        ...
        fn visit_unhandled(self, kind: UnhandledKind) -> Self::Value {
            let Some(f) = self.visit_unhandled else {
        ...
        fn visit_not_found(self) -> Self::Value {
            let Some(f) = self.visit_not_found else {
                return self.visit_unhandled(UnhandledKind::NotFound)
            };
            f()
        }
        ...
        where
            Fields: FieldIter<'resolver, Self::TypeId>,
        {
            let Some(f) = self.visit_composite else {
                return self.visit_unhandled(UnhandledKind::Composite)
            };
            f(&mut fields)
        }
        ...
        where
            ...
            Var: VariantIter<'resolver, Fields>,
        {
            let Some(f) = self.visit_variant else {
                return self.visit_unhandled(UnhandledKind::Variant)
            };
            ...
            let mut var_iter = variants.map(|v| {
                Variant {
                    index: v.index,
                    name: v.name,
                    fields: ConcreteFieldIter { fields: v.fields.collect(), idx: 0 }
                }
            });
        ...
        fn visit_sequence(self, type_id: &'resolver Self::TypeId) -> Self::Value {
            let Some(f) = self.visit_sequence else {
                return self.visit_unhandled(UnhandledKind::Sequence)
            };
            f(type_id)
        }

        fn visit_array(self, type_id: &'resolver Self::TypeId, len: usize) -> Self::Value {
            let Some(f) = self.visit_array else {
                return self.visit_unhandled(UnhandledKind::Array)
            };
            f(type_id, len)
        }
        ...
        where
            TypeIds: ExactSizeIterator<Item = &'resolver Self::TypeId>,
        {
            let Some(f) = self.visit_tuple else {
                return self.visit_unhandled(UnhandledKind::Tuple)
            };
            f(&mut type_ids as &mut dyn ExactSizeIterator<Item = &'resolver Self::TypeId>)
        }

        fn visit_primitive(self, primitive: Primitive) -> Self::Value {
            let Some(f) = self.visit_primitive else {
                return self.visit_unhandled(UnhandledKind::Primitive)
            };
            f(primitive)
        }

        fn visit_compact(self, type_id: &'resolver Self::TypeId) -> Self::Value {
            let Some(f) = self.visit_compact else {
                return self.visit_unhandled(UnhandledKind::Compact)
            };
            f(type_id)
        }
        ...
        fn visit_bit_sequence(
            ...
            let Some(f) = self.visit_bit_sequence else {
                return self.visit_unhandled(UnhandledKind::BitSequence)
            };
            f(store_format, order_format)
        }
    }
}